...
pub use builder::OssConfigBuilder;

use self::envvars::{apply_optional_env_overrides, require_env};
use self::tomlfile::{reject_secret_keys_in_toml, OssTomlFile};
...
"#;
    ...
    fn missing_toml_file_is_io_error() {
        let missing =
            std::env::temp_dir().join(format!("ossx-missing-{}.toml", std::process::id()));
