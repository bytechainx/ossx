...
impl OssHealth {
    fn unreachable(latency_ms: u64, detail: impl Into<String>) -> Self {
        Self {
            ready: false,
...
impl OssPool {
    ...
    async fn get_once(&self, key: &str) -> OssResult<Bytes> {
        let credentials = self.credentials().await?;
        let resource = sign::canonicalized_resource(&self.inner.config.bucket, key);
        let headers = signed_headers(
            &credentials.access_key_id,
    ...
    async fn delete_once(&self, key: &str) -> OssResult<()> {
        let credentials = self.credentials().await?;
        let resource = sign::canonicalized_resource(&self.inner.config.bucket, key);
        let headers = signed_headers(
            &credentials.access_key_id,
    ...
    async fn head_once(&self, key: &str) -> OssResult<ObjectMeta> {
        let credentials = self.credentials().await?;
        let resource = sign::canonicalized_resource(&self.inner.config.bucket, key);
        let headers = signed_headers(
            &credentials.access_key_id,
    ...
    async fn init_mp_once(&self, key: &str, sse: bool) -> OssResult<String> {
        let credentials = self.credentials().await?;
    ...
    async fn abort_mp_once(&self, key: &str, upload_id: &str) -> OssResult<()> {
        validate_upload_id(upload_id)?;
        let credentials = self.credentials().await?;
        let resource = sign::canonicalized_resource_with_subresources(
            &self.inner.config.bucket,
